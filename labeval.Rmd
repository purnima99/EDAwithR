...
```{r}
Literacy_rates <- read_excel("/home/purnima/Downloads/Literacy rates.xlsx")
View(Literacy_rates)
...
```
This visual shows countries with the highest average literacy rate in 2018 for females compared to the average of all previous years.
Quite a few countries here from South America with Argentine staying consistent around the 90% mark. Other LATAM Countries like Brazil, Mexico, and Colombia have shown good growth. Bangladesh has seen a phenomenal increase in female literacy rates.
...
```
The grey vertical line represents the global literacy rate average and those big red circles highlight the average literacy rate against each region. Regions like sub-Saharan Africa, northern and western Africa, central and southern Asia have a lower average literacy rate compared to the global average and other regions. Not surprising to see that people belonging to the older age group generally have a lower literacy rate.

## 03. Literacy Rate Trend across Age Groups
```{r, fig.height=7, fig.width=20}
...
```
Nothing interesting to see here except that there was a sharp decline in literacy rate for all of the age groups in 2012
...
literacy_data %>%
  ...
  theme(
    ...
    legend.position = "none")
```
I was curious to see the countries whose average female literacy rate was greater or equal to their male average literacy rate. Most of the countries on this graph are small countries belonging to the South American and African regions which is really surprising.
...
literacy_data %>%
  ...
  theme(
    ...
    legend.position = "none")
```
Next, I wanted to see the countries that had the highest discrepancy between male and female literacy rates. For aesthetics, I only visualized the top 20 based on the value of the difference between the two genders. Again, we can see a lot of African countries here as well and some countries from Asia like Pakistan — where I come from. It's very interesting to see that Africa has countries where the average female literacy rate is greater than the opposite gender but at the same time, there are countries in Africa where the male literacy rate is more than the female literacy rate by a huge margin.
...
```
Since I am from Pakistan, I wanted to see how my country compares to other countries in the same region. While the general trend is slightly upwards, it is disappointing to see that Pakistan is only doing better than Afghanistan and lagging behind its neighbors like India, Bangladesh, and Iran.

## 08. Taking a look at India
```{r, fig.height=5, fig.width=15}
